...
function UrlInput({ urls, handleGetShortnedUrls ,setUrls}) {
      ...
      .then(({ data }) => {
        ...
        if (ok && !isDuplicate) {
          ...
          if (window.location.href.indexOf("localhost") > -1) {
            axios
              .post("http://localhost:3004/urls", payload)
              .then((response) => {
                handleGetShortnedUrls();
              });
          }else{
            handleGetShortnedUrls(payload);
          }
        }
      });
  };
  return (
    ...
    >
      ...
      <Button onClick={handlePostUrl} marginLeft="1rem" colorScheme="blue">
        Get Short Url
      </Button>
    </Box>
  );
